...
resize.get(
  ...
  async (req: express.Request, res: express.Response): Promise<void> => {
    ...
    const path = `${bPath}/thumbnails/${fileName}_${width}_${height}.jpg`;
    let retString = ``;

    //create resized images path if not exists
    if (!existsSync(`${bPath}/thumbnails/`)) {
      mkdir(`${bPath}/thumbnails/`, { recursive: false }, (err) => {
        if (err) throw err;
      });
    }
    ...
    if (isNaN(height) && isNaN(width) && fileName === undefined) {
      ...
      try {
        ...
        //if so the resized file will be shown. Othernpm rwise use sharp to resize the photo
        if (existsSync(path)) {
          retString = `Image has already been processed.  Using file: ` + path;
        } else {
          //Call the resizing utility
          await resizingUtility(`${bPath}/full/${fileName}.jpg`, path, width, height );
